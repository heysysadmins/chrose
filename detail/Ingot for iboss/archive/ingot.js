    ...
    chrome.management.getAll(async function (allExtensions) {
        for (let anExtension in allExtensions)
            if (!allExtensions[anExtension].isApp || true) {
                if (savedata) {
                    if (
                        savedata.hasOwnProperty(allExtensions[anExtension].id)
                    ) {
                ...
                addExtension({
                    ...
                    managed:
                        allExtensions[anExtension].installType == "admin"
                            ? true
                            : false,
        ...
        if (item.getAttribute("data-id") == "proxy") {
            (await proxyEnabled())
                ? item
                      ...
                      .removeAttribute("unchecked")
                : item
                      .querySelector(".item-toggle")
                      .setAttribute("unchecked", "");
        } else {
            chrome.management.get(item.getAttribute("data-id"), (info) => {
                if (info.enabled) {
                    item.querySelector(".item-toggle").removeAttribute(
                        "unchecked",
                    );
                } else {
                    item.querySelector(".item-toggle").setAttribute(
                        "unchecked",
                        "",
                    );
                }
...
async function setIcons() {
    ...
    for (let item in items) {
        try {
            items[item].querySelector(
                ".item-main .item-img-wrapper .item-img",
            ).src = await getIconFromExtension(items[item].dataset.id);
        } catch {}
    }
}
