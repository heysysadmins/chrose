            ...
            if (
                ...
                var c = goog.Dependency.registerCallback_(function (f) {
                        (goog.DebugLoader_.IS_OLD_IE_ &&
                            ...
                            (goog.Dependency.unregisterCallback_(c),
                            a.loaded());
                    }),
                    d =
...
sre.SystemExternal.document = sre.SystemExternal.documentSupported()
    ? document
    : new sre.SystemExternal.xmldom.DOMImplementation().createDocument(
          "",
          "",
          0,
      );
...
sre.DynamicCstr.prototype.allProperties = function () {
    for (
        var a = sre.DynamicCstr.superClass_.allProperties.call(this),
            b = 0,
            c,
            d;
...
sre.SemanticUtil.hasDisplayTag = function (a) {
    return (
        !!a &&
        -1 !== sre.SemanticUtil.DISPLAYTAGS.indexOf(sre.DomUtil.tagName(a))
    );
};
...
sre.SemanticUtil.addAttributes = function (a, b) {
    if (b.hasAttributes()) {
        ...
        for (var c = b.length - 1; 0 <= c; c--) {
            var d = b[c].name;
            d.match(/^ext/) &&
                ((a.attributes[d] = b[c].value), (a.nobreaking = !0));
            ...
            "href" === d &&
                ((a.attributes.href = b[c].value), (a.nobreaking = !0));
        }
    }
};
...
sre.SemanticAttr.isOpeningFence = function (a) {
    return (
        a == sre.SemanticAttr.Role.OPEN || a == sre.SemanticAttr.Role.NEUTRAL
    );
};
sre.SemanticAttr.isClosingFence = function (a) {
    return (
        a == sre.SemanticAttr.Role.CLOSE || a == sre.SemanticAttr.Role.NEUTRAL
    );
};
...
sre.SemanticNode.prototype.toJson = function () {
    ...
    this.childNodes.length &&
        (a[sre.SemanticNode.Attribute.CHILDREN] = this.childNodes.map(
            function (e) {
                return e.toJson();
            },
        ));
...
sre.SemanticNode.processChildren_ = function (a, b) {
    ...
    for (var c = b.next(); !c.done; c = b.next())
        if (((c = c.value), c.nodeType === sre.DomUtil.NodeType.TEXT_NODE))
            ...
            var d = sre.DomUtil.toArray(c.childNodes).map(
                sre.SemanticNode.fromXml,
            );
...
sre.SemanticAnnotations.prototype.annotate = function (a) {
    for (
        var b = $jscomp.makeIterator(Object.keys(this.annotators)),
            c = b.next();
        !c.done;
        c = b.next()
    )
        (c = c.value),
            this.annotators[c].active && this.annotators[c].annotate(a);
    b = $jscomp.makeIterator(Object.keys(this.visitors));
    for (c = b.next(); !c.done; c = b.next())
        (c = c.value),
            this.visitors[c].active &&
                this.visitors[c].visit(
                    a,
                    Object.assign({}, this.visitors[c].def),
                );
};
...
sre.SemanticCollator_.prototype.reduce = function () {
    for (var a in this.map_)
        1 !== this.map_[a].length &&
            (this.map_[a] = sre.SemanticOrdering.getInstance().reduce(
                this.map_[a],
            ));
};
        ...
        method: function (a) {
            for (var b = a.childNodes.length - 1, c; (c = a.childNodes[b]); b--)
                sre.SemanticPred.isImplicitOp(c) &&
                    !c.nobreaking &&
                    (a.childNodes.splice.apply(
                        a.childNodes,
                        [b, 1].concat(c.childNodes),
                    ),
...
sre.SemanticHeuristics.add(
    ...
    new sre.SemanticTreeHeuristic({
        method: function (a) {
            if (
                !(
                    a.role !== sre.SemanticAttr.Role.UNKNOWN ||
...
sre.SemanticHeuristics.add(
    ...
    new sre.SemanticMultiHeuristic({
        method: function (a) {
            ...
            b = b.rel.length
                ? sre.SemanticHeuristics.juxtapositionPrePost_(b)
                : b;
            a = b.comp[0];
...
sre.SemanticProcessor.prototype.implicitNode_ = function (a) {
    var b =
        sre.SemanticProcessor.getInstance().factory_.makeMultipleContentNodes(
            a.length - 1,
...
sre.SemanticProcessor.getSpacer_ = function (a) {
    ...
    for (; sre.SemanticUtil.hasEmptyTag(a) && 1 === a.childNodes.length; )
        if (((a = a.childNodes[0]), "MSPACE" === sre.DomUtil.tagName(a)))
            return a;
    return null;
};
...
sre.SemanticProcessor.prototype.operationsTree_ = function (a, b, c, d) {
    ...
    return a.div
        ? sre.SemanticProcessor.getInstance().operationsTree_(
              a.tail,
              b,
              a.div,
              [],
...
sre.SemanticProcessor.prototype.fences_ = function (a, b, c, d) {
    ...
    if (0 === a.length) {
        for (a = d.shift(); 0 < c.length; ) {
            if (e(c[0]))
                (b = c.shift()),
                    sre.SemanticProcessor.fenceToPunct_(b),
                    a.push(b);
            else {
    ...
    e &&
        ...
        f &&
            (this.accentRole_(f, sre.SemanticAttr.Type.OVERSCORE) && !e
                ? ((g =
                      sre.SemanticProcessor.getInstance().factory_.makeBranchNode(
                          sre.SemanticAttr.Type.OVERSCORE,
                          [a, f],
                          [],
                      )),
...
sre.SemanticProcessor.prototype.getFunctionsInRow_ = function (a, b) {
    ...
    if (!d)
        return (
            b.push(c),
            sre.SemanticProcessor.getInstance().getFunctionsInRow_(a, b)
        );
    a = sre.SemanticProcessor.getInstance().getFunctionsInRow_(a, []);
...
sre.SemanticProcessor.prototype.getFunctionArgs_ = function (a, b, c) {
    switch (c) {
        case "integral":
            ...
            if (!b.intvar && !b.integrand.length)
                return b.rest.unshift(a), b.rest;
        ...
        case "prefix":
            ...
            if (c.head.length)
                ...
                if (
                    !c.div ||
                    !sre.SemanticPred.isAttribute("type", "APPL")(c.div)
                )
...
sre.SemanticProcessor.tableToMultiline = function (a) {
    if (sre.SemanticPred.tableIsMultiline(a)) {
        ...
        for (var b = 0, c; (c = a.childNodes[b]); b++)
            sre.SemanticProcessor.rowToLine_(
                c,
...
sre.SemanticProcessor.prototype.mfenced = function (a, b, c, d) {
    if (c && 0 < d.length) {
        ...
        d.forEach(
            goog.bind(function (g) {
                f.push(
                    sre.SemanticProcessor.getInstance().factory_.makeContentNode(
                        e(),
                    ),
                );
                f.push(g);
    ...
    b &&
        d.push(sre.SemanticProcessor.getInstance().factory_.makeContentNode(b));
...
sre.SemanticProcessor.rewriteFence_ = function (a, b) {
    ...
    if (
        ...
        return (
            sre.SemanticPred.isAttribute("role", "SUBSUP")(b) ||
                (b.role = a.role),
...
sre.SemanticMathml.prototype.action_ = function (a, b) {
    return 1 < b.length
        ? this.parse(b[1])
        : this.getFactory().makeUnprocessed(a);
};
...
sre.MathspeakUtil.getNestingDepth = function (a, b, c, d, e, f) {
    ...
    sre.MathspeakUtil.nestingDepth[a] ||
        (sre.MathspeakUtil.nestingDepth[a] = {});
...
sre.MathspeakUtil.computeNestingDepth_ = function (a, b, c, d, e, f) {
    if (
        e(a) ||
        -1 < c.indexOf(a.tagName) ||
        sre.MathspeakUtil.containsAttr(a, d)
    )
        return f;
...
sre.MathspeakUtil.nestedSubSuper = function (a, b, c) {
    for (; a.parentNode; ) {
        ...
        if (
            (e.tagName === sre.Semantic.Type.SUBSCRIPT &&
                a === d.childNodes[1]) ||
        ...
        if (
            (e.tagName === sre.Semantic.Type.SUPERSCRIPT &&
                a === d.childNodes[1]) ||
            (e.tagName === sre.Semantic.Type.TENSOR &&
                f &&
...
sre.MathspeakUtil.generateBaselineConstraint = function () {
    var a = function (g) {
        ...
        b =
            "not(" +
            a(["subscript", "superscript", "tensor"]).join(" or ") +
            ")",
        c = a(["relseq", "multrel"]);
...
sre.Locale.de = {
    ...
    MS_FUNC: {
        ...
        RADICAL_NEST_DEPTH: function (a) {
            return 1 < a
                ? sre.Numbers.de.NUMBERS.numberToWords(a) + "fach"
                : "";
        },
        COMBINE_ROOT_INDEX: function (a, b) {
    ...
    FONT: {
        ...
        "double-struck-italic": [
            "kursiv mit Doppelstrich",
            germanPostfixCombiner,
        ],
        "fraktur": "Fraktur",
        ...
        if (c) a += c;
            ...
            d.match(/\-dix$/)
                ...
                : (a +=
                      d +
                      (b % 10
                          ? "-" + sre.Numbers.fr.onesNumbers_[b % 10]
                          : ""));
        }
    }
    ...
    if ((a %= 100)) {
        ...
        if (c) b += c;
            ...
            b =
                ...
                (d
                    ? sre.Numbers.it.NUMBERS.numSep +
                      sre.Numbers.it.onesNumbers_[a % 10]
                    : "");
        }
    }
    return b;
...
sre.Numbers.it.wordOrdinal = function (a) {
    var b =
            "male" === sre.Grammar.getInstance().getParameter("gender")
                ? "o"
                : "a",
...
sre.SemanticSkeleton.prototype.populate_ = function (a, b, c) {
    if (sre.SemanticSkeleton.simpleCollapseStructure(a))
        (this.levelsMap[a] = b),
            (this.parents[a] = a === c[0] ? c.slice(1) : c);
    else {
...
sre.SemanticSkeleton.makeSexp_ = function (a) {
    return sre.SemanticSkeleton.simpleCollapseStructure(a)
        ...
        : sre.SemanticSkeleton.contentCollapseStructure(a)
          ? "(c " +
            a.slice(1).map(sre.SemanticSkeleton.makeSexp_).join(" ") +
            ")"
...
sre.SemanticSkeleton.addOwns_ = function (a, b) {
    ...
    b = c
        ? sre.SemanticSkeleton.realLeafs_(
              sre.SemanticSkeleton.fromString(c).array,
          )
        : b.map(function (d) {
              return d.id;
          });
...
sre.EnrichMathml.functionApplication_ = function (a, b) {
    ...
    if (
        ...
        for (var d; (d = a.attributes[c]); c++)
            b.hasAttribute(d.nodeName) ||
                b.setAttribute(d.nodeName, d.nodeValue);
...
sre.EnrichMathml.pathToRoot_ = function (a, b) {
    ...
    for (
        var c = [a];
        !b(a) && !sre.SemanticUtil.hasMathTag(a) && a.parentNode;
...
sre.EnrichMathml.isIgnorable_ = function (a) {
    ...
    return (!sre.SemanticUtil.hasEmptyTag(a) && b.length) ||
        ...
        ? !1
        : sre.DomUtil.toArray(a.childNodes).every(
              sre.EnrichMathml.isIgnorable_,
          );
};
...
sre.CaseMultiscripts.prototype.getMathml = function () {
    ...
    if (
        ...
        var a = this.semantic.childNodes[0],
            b = a.childNodes[0],
            c = sre.CaseMultiindex.multiscriptIndex(
...
sre.CaseEmbellished.prototype.fullFence = function (a) {
    for (
        var b = this.fencedMml.parentNode;
        a.parentNode && a.parentNode !== b;
...
sre.Enrich.testTranslation__ = function (a) {
    ...
    a = sre.Enrich.semanticMathmlSync(
        sre.Enrich.prepareMmlString(a),
    ).toString();
    sre.EnrichMathml.removeAttributePrefix(a);
    ...
    if (a.type === sre.SemanticAttr.Type.PUNCTUATED)
        return (
            sre.RebuildStree.isPunctuated(b) &&
                ...
                (a.contentNodes = b.map(
                    goog.bind(this.makePunctuation, this),
                ))),
            a
        );
...
sre.AudioUtil.personalityDiff_ = function (a, b) {
    ...
    if (sre.AudioUtil.isMarkupElement(c)) {
        for (a = c.close.slice(); 0 < a.length; ) {
            ...
            h = d;
            if (0 === a.length)
                0 !== h.length && sre.AudioUtil.LastOpen_.push(h);
            else if (0 !== h.length)
                for (
                    c.close = c.close.concat(h),
                        c.open = c.open.concat(h),
                        d = 0;
                    (e = h[d]);
                    d++
                )
...
sre.AbstractAudioRenderer.prototype.getSeparator = function () {
    return "braille" === sre.Engine.getInstance().modality
        ? ""
        : this.separator_;
};
            ...
            if (e.close.length)
                for (var f = 0; f < e.close.length; f++) {
                    ...
                    if (-1 === e.close.indexOf(g))
                        throw new sre.Engine.Error(
                            "Unknown closing markup element: " + g,
                        );
...
sre.AuralRendering.prototype.setSeparator = function (a) {
    var b =
        sre.AuralRendering.rendererMapping_[sre.Engine.getInstance().markup];
    b && b.setSeparator(a);
};
...
sre.SpeechRule.Component.attributesFromString = function (a) {
    if ("(" != a[0] || ")" != a.slice(-1))
        throw new sre.SpeechRule.OutputError(
            "Invalid attribute expression: " + a,
        );
    var b = {};
...
sre.TrieNodeFactory.constraintTest_ = function (a) {
    ...
    if (a.match(/^@\w+="[\w\d ]+"$/)) {
        ...
        return function (g) {
            return (
                g.hasAttribute && g.hasAttribute(d) && g.getAttribute(d) === e
            );
        };
    }
    if (a.match(/^@\w+!="[\w\d ]+"$/))
...
sre.BaseRuleStore.prototype.defineRule = function (a, b, c, d, e) {
    try {
        ...
        if ("RuleError" == m.name)
            return (
                console.error("Rule Error ", d, "(" + b + "):", m.message), null
            );
        throw m;
    }
    this.addRule(l);
...
sre.BaseRuleStore.prototype.evaluateDefault = function (a) {
    ...
    return a.match(/^\s+$/)
        ? this.evaluateWhitespace(a)
        : this.evaluateString(a);
};
...
sre.BaseRuleStore.prototype.removeDuplicates = function (a) {
    for (
        var b = this.speechRules_.length - 1, c;
        (c = this.speechRules_[b]);
        b--
    )
        c != a &&
...
sre.BaseRuleStore.comparePreconditions_ = function (a, b) {
    ...
    return a.query != b.query
        ? !1
        : sre.BaseRuleStore.compareStaticConstraints_(
              a.constraints,
              b.constraints,
          );
};
...
sre.MathStore.prototype.matchNumber_ = function (a) {
    ...
    return (b && b[0] === a) || !d
        ...
        : {
              number: c[0]
                  .replace(
                      new RegExp(sre.Locale.en.REGEXP.DIGIT_GROUP, "g"),
                      "X",
                  )
                  .replace(
                  ...
                  .replace(
                      /X/g,
                      sre.Messages.REGEXP.DIGIT_GROUP.replace(/\\/g, ""),
                  ),
              length: c[0].length,
          };
};
...
sre.ClearspeakPreferences.PREFERENCES = new sre.DynamicProperties({
    ...
    Matrix: "Auto Combinatoric EndMatrix EndVector SilentColNum SpeakColNum Vector".split(
        " ",
    ),
    ...
    for (c in a)
        if (a[c].speech && a[c].speech.clearspeak) {
            ...
            for (f in sre.ClearspeakPreferences.PREFERENCES.getProperties()) {
                var g =
                        sre.ClearspeakPreferences.PREFERENCES.getProperties()[
                            f
                        ],
                    h = [f + "_Auto"];
                if (g) {
...
sre.MathCompoundStore.prototype.addSiUnitRules = function (a) {
    for (
        var b = $jscomp.makeIterator(Object.keys(this.siPrefixes)),
            c = b.next();
        !c.done;
        c = b.next()
    ) {
        ...
        for (
            ...
            c.mappings[g] = {};
            for (
                var h = $jscomp.makeIterator(Object.keys(a.mappings[g])),
                    k = h.next();
                !k.done;
                k = h.next()
            )
                (k = k.value),
                    (c.mappings[g][k] = sre.Locale[this.locale].SI(
                        e,
                        a.mappings[g][k],
                    ));
        }
...
sre.SemanticAnnotations.getInstance().register(
    new sre.SemanticVisitor(
        "nemeth",
        "number",
        sre.NemethUtil.propagateNumber,
        {number: !0},
    ),
);
...
sre.NemethUtil.relationIterator = function (a, b) {
    var c = a.slice(0),
        ...
        e =
            0 < a.length
                ? sre.XpathUtil.evalXPath("../../content/*", a[0])
                : [];
    return function () {
        var f = e.shift(),
            g = c.shift(),
        ...
        g =
            ...
            (d &&
                ...
                f.parentNode.parentNode.previousSibling)
...
sre.StoreUtil.contentIterator = function (a, b) {
    var c =
        0 < a.length ? sre.XpathUtil.evalXPath("../../content/*", a[0]) : [];
    return function () {
        var d = c.shift(),
            e = b
                ? [sre.AuditoryDescription.create({text: b}, {translate: !0})]
        ...
        (sre.ClearspeakUtil.isLetter_(a.childNodes[0]) ||
            ...
            (a.childNodes[0].type === sre.SemanticAttr.Type.PREFIXOP &&
                ...
                (sre.ClearspeakUtil.isLetter_(a.childNodes[0].childNodes[0]) ||
                    sre.ClearspeakUtil.isNumber_(
                        a.childNodes[0].childNodes[0],
                    ))))
    );
};
...
sre.ClearspeakUtil.isSimpleNumber_ = function (a) {
    return (
        sre.ClearspeakUtil.isNumber_(a) ||
...
sre.UnitUtil.rightMostUnit = function (a) {
    for (; a; ) {
        ...
        a =
            -1 !== sre.UnitUtil.SCRIPT_ELEMENTS.indexOf(b)
                ? a[0]
                : a[a.length - 1];
    }
    return !1;
...
sre.SpeechRuleEngine.prototype.evaluateTree_ = function (a) {
    ...
    for (var g = 0, h; (h = f[g]); g++) {
        ...
        k[0] &&
            ...
            (m.context &&
                (k[0].context =
                    e.constructString(a, m.context) + (k[0].context || "")),
...
sre.SpeechGeneratorUtil.addSpeech = function (a, b, c) {
    b = (c = sre.DomUtil.querySelectorAllByAttrValue(
        c,
        "id",
        b.id.toString(),
    )[0])
...
sre.SpeechGeneratorUtil.connectMactions = function (a, b, c) {
    ...
    for (var d = 0, e; (e = b[d]); d++) {
        ...
        if ((f = sre.DomUtil.querySelectorAllByAttrValue(a, "id", f)[0])) {
            ...
            (h &&
                "dummy" !== h.getAttribute(sre.EnrichMathml.Attribute.TYPE)) ||
                ((h = f.childNodes[0]),
        ...
        (f = Array.isArray(e)
            ? e
                  .map(
                      (function (g) {
                          return function (h) {
                              return b.colorLeave_(
                                  a,
                                  h,
                                  g.$jscomp$loop$prop$color$12,
                              );
                          };
                      })(d),
                  )
...
sre.NodeSpeechGenerator.prototype.getSpeech = function (a, b) {
    ...
    return c
        ? c
        : sre.NodeSpeechGenerator.superClass_.getSpeech.call(this, a, b);
};
...
sre.SemanticWalker.prototype.combineContentChildren = function (a, b, c, d) {
    switch (a) {
        ...
        case sre.SemanticAttr.Type.APPL:
            return [
                this.focusFromId(d[0], [d[0], c[0]]),
                this.singletonFocus(d[1]),
            ];
...
sre.TableWalker = function (a, b, c, d) {
    ...
    this.modifier = !1;
    this.keyMapping[sre.EventUtil.KeyCode["0"]] = goog.bind(
        this.jumpCell,
        this,
    );
    this.keyMapping[sre.EventUtil.KeyCode["1"]] = goog.bind(
        this.jumpCell,
        this,
    );
    this.keyMapping[sre.EventUtil.KeyCode["2"]] = goog.bind(
        this.jumpCell,
        this,
    );
    this.keyMapping[sre.EventUtil.KeyCode["3"]] = goog.bind(
        this.jumpCell,
        this,
    );
    this.keyMapping[sre.EventUtil.KeyCode["4"]] = goog.bind(
        this.jumpCell,
        this,
    );
    this.keyMapping[sre.EventUtil.KeyCode["5"]] = goog.bind(
        this.jumpCell,
        this,
    );
    this.keyMapping[sre.EventUtil.KeyCode["6"]] = goog.bind(
        this.jumpCell,
        this,
    );
    this.keyMapping[sre.EventUtil.KeyCode["7"]] = goog.bind(
        this.jumpCell,
        this,
    );
    this.keyMapping[sre.EventUtil.KeyCode["8"]] = goog.bind(
        this.jumpCell,
        this,
    );
    this.keyMapping[sre.EventUtil.KeyCode["9"]] = goog.bind(
        this.jumpCell,
        this,
    );
    this.key_ = null;
    ...
    postprocessor: function (a, b) {
        ...
        switch (sre.Engine.getInstance().speech) {
            ...
            case sre.Engine.Speech.DEEP:
                (c = sre.SpeechGeneratorFactory.generator("Tree")),
                    c.getSpeech(b, a);
        }
        return a;
    },
            ...
            c
                ? sre.SystemExternal.fs.writeFile(c, d, function (e) {
                      if (e)
                          throw (
                              (this.files_--,
                              new sre.Engine.Error(
                                  "Can not write to file: " + c,
                              ))
                          );
                  })
                : console.info(d);
