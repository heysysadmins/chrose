(function () {
    if (window.chrome) {
        chromeVersion = navigator.userAgent.substring(
            navigator.userAgent.lastIndexOf("Chrome/"),
            navigator.userAgent.lastIndexOf(" "),
        );
        chromeVersion = chromeVersion
            .substring(chromeVersion.indexOf("/"), chromeVersion.indexOf("."))
            .replaceAll("/", "");
    } else {
        chromeVersion = "0";
    }
    const securlyVersion =
        document.getElementById("securly-version").textContent;
    ...
    */
    ...
    if (sessionStorage.getItem("version") == undefined) {
        if (chromeVersion > maxVersion) {
            ...
            document.getElementById("version").textContent =
                document.getElementsByClassName("versionBtn")[1].textContent;
        } else {
            if (chromeVersion < minVersion) {
                if (chromeVersion > 0) {
                    /* Chrome instance */ document.getElementById(
                        "version",
                    ).textContent = document
                        .getElementsByClassName("versionBtn")
                        .at(-1).textContent;
                } else {
                    document.getElementById("version").textContent = "All";
                }
            } else {
                document.getElementById("version").textContent = chromeVersion;
            }
            if (chromeVersion > 0) {
                document.getElementById(
                    `versionBtn-${chromeVersion}`,
    ...
    if (!localStorage.getItem("welcome")) {
        ...
        logo.appendChild(document.createTextNode("EXT-REMOVER"));
        summary = Object.assign(document.createElement("p"), {
            textContent:
                "EXT-REMOVER contains a collection of exploits discovered by various users to expand the capability of managed Chromebooks, making it easy to find the information and tools needed.",
        });
        disclaimer1 = Object.assign(document.createElement("p"), {
            textContent:
                "Exploit details, code, and styles have been modified for a better user experience.",
        });
        ...
        continue_button = Object.assign(document.createElement("button"), {
            className: "continue",
    ...
    document
        ...
        .addEventListener("mousedown", selectVersion);
    ...
    function filterOptions(version) {
        ...
        for (let i = 0; i < options.length; i++) {
            let patch = document.getElementsByClassName("patch")[i].textContent,
                patch_securly,
                patch_securly_elem;
            if (
                (patch_securly_elem ==
                    options[i].getElementsByClassName("patch-securly"))[0]
            ) {
                patch_securly = patch_securly_elem.textContent;
            }
            if (
                ((version >= patch || securlyVersion >= patch_securly) &&
                    version !== "none") ||
                patch == "Hidden"
            ) {
        ...
        document
            .getElementById(
                `versionBtn-${parseInt(version) ? version : "none"}`,
            )
    ...
    function thumbnailHide(hide) {
        ...
        for (let i = 0; i < thumbnails.length; i++) {
            ...
            thumbnails[i].parentElement.style.height = hide
                ? "100px"
                : thumbnails[i].parentElement.style.maxHeight;
        }
