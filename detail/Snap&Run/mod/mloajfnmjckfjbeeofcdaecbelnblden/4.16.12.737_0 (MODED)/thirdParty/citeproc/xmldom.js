...
CSL_CHROME.prototype.getNodeValue = function (myxml, name) {
    ...
    if (
        ret &&
        ret.childNodes &&
        (ret.childNodes.length == 0 ||
            (ret.childNodes.length == 1 && ret.firstChild.nodeName == "#text"))
    ) {
...
CSL_CHROME.prototype.setAttributeOnNodeIdentifiedByNameAttribute = function (
    myxml,
    nodename,
    partname,
    attrname,
    ...
    for (pos = 0, len = nodes.length; pos < len; pos += 1) {
        ...
        if (
            ...
            !(
                ...
                node.getAttribute("name") == nameattrval
            )
        ) {
            continue;
        }
...
CSL_CHROME.prototype.insertChildNodeAfter = function (
    parent,
    node,
    pos,
    datexml,
    ...
    for (var i = 0, ilen = group.length; i < ilen; i += 1) {
        ...
        if (node.childNodes.length - skippers.length === 2) {
            ...
            for (var j = 0, jlen = 2; j < jlen; j += 1) {
                ...
                var subskippers = [];
                for (
                    var k = 0, klen = child.childNodes.length;
                    k < klen;
                    k += 1
                ) {
...
CSL_CHROME.prototype.addInstitutionNodes = function (myxml) {
    ...
    for (pos = 0, len = names.length; pos < len; pos += 1) {
        ...
        if (institution.length == 0) {
            theinstitution = this.importNode(
                myxml.ownerDocument,
                this.institution,
            );
            ...
            for (
                var j = 0, jlen = CSL.INSTITUTION_KEYS.length;
                j < jlen;
                j += 1
            ) {
